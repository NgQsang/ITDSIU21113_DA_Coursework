...
data_new_1 <- subset(data_origin_1, select = -c(Metro, State,CountyName, City))
colnames(data_new_1) <- sub("^X", "", colnames(data_new_1)) #remove prefix X in the dates

#Using pivot_longer to make the data set less large (include all date values in one column).
data_new_pivot1 <- data_new_1 %>%
  pivot_longer(
